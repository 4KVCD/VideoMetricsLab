...
def test_store_then_load_cached_round_trips(tmp_path):
    ...
    assert result_cache.load_cached(source, distorted) is None  # nothing cached yet

    result = _fake_result(source, distorted)
    result_cache.store(source, distorted, result, label="my-run")

    loaded = result_cache.load_cached(source, distorted)
...
def test_cache_miss_when_distorted_file_size_differs(tmp_path):
    ...
    result_cache.store(source, distorted_v1, _fake_result(source, distorted_v1), label="v1")

    # Same name, different size (e.g. the file was re-encoded) -- must miss.
    distorted_v2 = _make_file(tmp_path / "distorted.mp4", 999)
    assert result_cache.load_cached(source, distorted_v2) is None
...
def test_clear_removes_the_cached_entry(tmp_path):
    source = _make_file(tmp_path / "source.mp4", 1000)
    distorted = _make_file(tmp_path / "distorted.mp4", 500)
    result_cache.store(source, distorted, _fake_result(source, distorted), label="x")
    assert result_cache.load_cached(source, distorted) is not None

    result_cache.clear(source, distorted)
    assert result_cache.load_cached(source, distorted) is None
...
def test_clear_on_nonexistent_entry_does_not_raise(tmp_path):
    source = _make_file(tmp_path / "source.mp4", 1000)
    distorted = _make_file(tmp_path / "distorted.mp4", 500)
    result_cache.clear(source, distorted)  # never stored -- must not raise
